...
The sample average of μ̂ from only the tests in which the null is rejected does not match the true μ when the effect size is small. Although μ̂ is unbiased overall, restricting attention to significant results introduces an upward bias, because rejection is more likely when μ̂ happens to be unusually large, producing selection bias.
...
```
The raw dataset contains 52,179 homicide cases from 50 large U.S. cities, with each row representing a single victim. The variables include city and state identifiers, victim demographics (age, sex, race), geographic coordinates, the date of the homicide, and the case disposition. The disposition variable indicates whether the case was “Closed by arrest” (solved), “Closed without arrest,” or “Open/No arrest” (both unsolved). This structure allows for city-level comparisons of homicide characteristics and case outcomes.
